...
<?php if (!$ajax) {  ?>
  ...
  <div id="energyplus-orders-1">
    ...
    <?php } else {  ?>
        ...
        <?php  foreach ($orders AS $order_group) {  ?>
          ...
          <div class="__A__Orders_Container">
            <?php foreach ($order_group['orders'] AS $order) {  ?>
              <div class="btnA __A__Item collapsed"  data-toggle="collapse" data-target="#order_<?php echo esc_attr($order['id'])?>" aria-expanded="false" aria-controls="order_<?php echo esc_attr($order['id'])?>"  id="item_<?php echo esc_attr($order['id'])?>" rel-status="<?php echo esc_attr($order['status'])?>">
                <div class="liste  row d-flex align-items-center">
                  ...
                  <div class="text-center d-none d-sm-inline __A__Order_No" data-colname="<?php esc_html_e('Order No: ', 'energyplus'); ?>"><span class="__A__Order_No __A__Strong eix-quick"  href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>"><?php echo esc_attr($order['std']->get_order_number())?></span></div>
                  <div class="col col-sm-2 col-md-1 energyplus-orders--item-badge text-center __A__Col_3">

                    <span class="orderstatusx text-<?php echo esc_attr($order['status']);?>"><span class="bg-custom bg-<?php echo esc_attr($order['status']);?>" aria-hidden="true"></span><br><?php echo wc_get_order_status_name($order['status']); ?></span>

                    <span class="badge badge-pill __A__Display_None"><?php echo esc_html(wc_get_order_status_name($order['status'])); ?></span></div>
                    <div class="__A__Col_Name col-7 col-sm-2"><p class="energyplus-orders--name eix-quick" href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>">
                    ...
                    <p class="energyplus-orders--address eix-quick"  href="<?php echo admin_url( 'post.php?post=' . esc_attr($order['id']). '&action=edit&energyplus_hide' );?>">
                      <?php if (isset($order['shipping']['country']))  {
                        ...
                        if ('' !== EnergyPlus_Helpers::clean($order['shipping']['state'], $order['billing']['state'])) {
                            echo ', ';
                        }
                  ...
                  <div class="col col-sm-4 d-md-none d-lg-block __A__Order_Products __A__Col_3" data-colname='<?php esc_html_e('Products', 'energyplus'); ?>'>
                    <?php foreach ($order['line_items'] AS $item) {  ?>
                      <?php echo EnergyPlus_Helpers::product_image($item['product_id'],  $item['quantity']+$order['std']->get_qty_refunded_for_item($item->get_id()), 'width: 55px;'); ?>
                    <?php } ?>
                  ...
                  <div class="col __A__Col_Price __A__Col_3X text-right" data-colname='<?php esc_attr_e('Total', 'energyplus')?>'>
                  ...
                  <div class="row __A__Order_Items">
                    <div class="col-md-4 col-sm-6">
                      <?php  foreach ($order['line_items'] AS $item) {?>
                        <div class="row __A__Order_Item">
                          ...
                          <div class="col-9 col-sm-9 col-md-10">
                            ...
                            <div class="pricex">
                              <?php
                                $refund_qty = $order['std']->get_qty_refunded_for_item($item->get_id());
                                $refund_total = $order['std']->get_total_refunded_for_item($item->get_id());
                                echo wc_price(($item['subtotal']/$item['qty']), array('currency' => $order['currency'])); ?> x   <span class="badge badge-pill badge-danger"><?php echo esc_html($item['qty']+$refund_qty); ?></span> =   <?php echo wc_price($item['subtotal']-$refund_total, array('currency' => $order['currency']));
                                if (0 !== $refund_qty) {
                                  echo '<br><span class="badge badge-pill badge-item-trash"><small>'.esc_html__('Refunded', 'energyplus').'</small></span>';
                                }

                                do_action('energyplus_after_order_line_item', $item);
                        ...
                        <?php foreach ($order['std']->get_items( 'coupon' ) as $coupon_item_id => $coupon_item ) {
                          ...
                          <div class="row __A__Order_Item">
                            ...
                            <div class="col-9 col-sm-9 col-md-10">
                              <h4 class="text-uppercase"><?php echo esc_html($coupon_item->get_code())?></h4>

                              <div class="pricex">
                                - <?php echo wc_price($coupon_item->get_discount(), array('currency' => $order['currency'])); ?>
                              </div>
                            </div>
                          </div>
                        <?php } ?>
                        ...
                        <?php $total_refunded = $order['std']->get_total_refunded();
                        if (0 < $total_refunded) { ?>
                          <div class="row __A__Order_Item">
                            <div class="col-3 col-sm-3 col-md-2"><div  class="__A__Order_Item_Group"  >»</div></div>
                            <div class="col-9 col-sm-9 col-md-10">
                              <h4><?php echo esc_html__('Refunded', 'energyplus')?></h4>

                              <div class="pricex">
                                - <?php echo wc_price($total_refunded, array('currency' => $order['currency'])); ?>
                              </div>
                            </div>
                          </div>
                          ...
                          <div class="row __A__Order_Item">
                            <div class="col-3 col-sm-3 col-md-2"><div  class="__A__Order_Item_Group"  >»</div></div>
                            <div class="col-9 col-sm-9 col-md-10">
                              <h4><?php echo esc_html__('Net', 'energyplus')?></h4>

                              <div class="pricex">
                                <?php echo wc_price($order['std']->get_total()-$total_refunded, array('currency' => $order['currency'])); ?>
                              </div>
                            </div>
                          </div>
                        ...
                        <?php if ($order['customer_note']) { ?>
                          <div class="__A__Order_Customer_Notice mb-4 w-100 d-inline-block d-sm-none "><?php printf(esc_html__('Note: %s', 'energyplus'), esc_html($order['customer_note']))?></div>
                        <?php } ?>

                        <?php do_action('energyplus_order_details', intval($order['id'])) ?>

                      </div>
                      <div class="col-sm-1"></div>
                      <div class="col-md-4 col-sm-5 __A__StopPropagation __A__Order_Address">

                        <?php do_action('energyplus_before_order_addresses', intval($order['id'])) ?>
                        ...
                        <?php do_action('energyplus_after_order_addresses', intval($order['id'])) ?>
                      ...
                      <div class="col-md-2 col-sm-12 __A__Order_Address __A__Order_Actions">
                        <?php  if (!empty($order['next_statuses']) && 'trash' !== $order['status']) {  ?>
                          <div class="row">
                            ...
                            <?php foreach ($order['next_statuses'] AS $next_status) {
                              if (isset($order_statuses[$next_status])) { ?>
                        ...
                        <?php } ?>
                        <div class="row">
                          <?php do_action('energyplus_before_order_actions', intval($order['id'])) ?>
                          ...
                          <?php } else {  ?>
                            ...
                            <?php } ?>
                          <?php } ?>
                          <?php do_action('energyplus_after_order_actions', intval($order['id'])) ?>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <?php if (!$ajax || ($ajax && 'true' === EnergyPlus_Helpers::post('order_refresh', 'false'))) {
        $count_status = EnergyPlus_Helpers::get('status', '');
        if ('' === $count_status) {
          $count_status =  EnergyPlus_Helpers::post('status', '');
        }
        if ('' === $count_status) {
          $count_status = 'any';
        }
        ?>
        <?php echo EnergyPlus_View::run( 'core/pagination', array( 'count' => $list['statuses_count'][$count_status], 'url'=>EnergyPlus_Helpers::admin_page('orders', array('status'=>EnergyPlus_Helpers::get('status', EnergyPlus_Helpers::post('status', 'any')))), 'per_page'=> absint(EnergyPlus::option('reactors-tweaks-pg-orders', 10)), 'page' => intval ( EnergyPlus_Helpers::get( 'pg', 0 ) ) )); ?>
      </div>
    </div>
  <?php } ?>
