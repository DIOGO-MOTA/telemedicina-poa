jQuery(document).ready(function() {
  ...
  /* Uncomment this block if you have a plugin which use an old version of Bootstrap */
  /*
  jQuery('body').on('click', '.__A__Item.btnA', function(e) {
     e.stopPropagation();
     e.preventDefault();
     console.log('ENERGY DEBUG: item clicked');
     jQuery(this).toggleClass('collapsed');
     jQuery(this).find('> div:not(.liste)').toggleClass('collapse show');
   });
   */
  ...
  */
  ...
  var trig =  jQuery('#slider').slideReveal({
    ...
    hidden: function(slider, trigger){
      ...
      if (1 === window.refreshOnClose && 1 === window.refreshOnClose2 && !window.isMobile) {
  ...
  jQuery( "body" ).on( "click", '.trig', function(e) {
    if (!window.isMobile) {
      ...
      if (jQuery(this).hasClass('no-refresh')) {
        window.refreshOnClose2 = 0;
      }
      ...
      if (jQuery(this).attr('data-width')) {
        if (window.device_width > parseInt(jQuery(this).attr('data-width')) || jQuery(window).width()<parseInt(jQuery(this).attr('data-width'))) {
          jQuery('#slider').css({width: window.device_width});
        } else {
        jQuery('#slider').css({width:  jQuery(this).attr('data-width')});
      }
      } else {
        jQuery('#slider').css({width: window.device_width});
      }
    ...
    jQuery(document).on('keydown', function(event) {
      ...
      var excludeInputs = [
        ...
        if(event.which === 65) {
          ...
          if(event.which === 81) {
            window.location = EnergyPlusGlobal._admin_url + '?dashboard=yes';
          }
      }
    );
  }
  ...
  /* Quick Link */
if ("1" === EnergyPlusGlobal.reactors_tweaks_quicklinks) {
    jQuery('body').on('click', '.eix-quick', function (e) {
      if (!window.isMobile) {
        e.preventDefault();
        e.stopPropagation();
