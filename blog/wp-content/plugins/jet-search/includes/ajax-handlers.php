	...
	class Jet_Search_Ajax_Handlers {
		...
		public function init() {
			...
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
				...
				add_action( 'wp_ajax_jet_search_save_query_control_settings', array( $this, 'save_query_control_settings' ) );
				add_action( 'wp_ajax_jet_search_load_query_control_settings', array( $this, 'load_query_control_settings' ) );
			}
		...
		*/
		 ...
		 * @return void
		 */
		public function save_query_control_settings() {
			$data = $_REQUEST;
			...
			if ( false === get_option( 'jet_ajax_search_query_settings' ) ) {
				...
				update_option( 'jet_ajax_search_query_settings', $settings );
			}
		 ...
		 * @return void
		 */
		public function load_query_control_settings() {
			$resultData['settings'] = get_option( 'jet_ajax_search_query_settings' );
		...
		public function cf_search_clauses( $args, $query ) {
			...
			if ( ( $key = array_search( '_sku', $cf_keys ) ) !== false ) {
				if ( function_exists( 'WC' ) ) {
					...
					$search = esc_sql( $wpdb->esc_like( $query->query['s'] ) );

					$cf_sku_where = " OR ( {$this->postmeta_table_alias}.meta_key = '_sku' AND {$this->postmeta_table_alias}.meta_value LIKE '%{$search}%' AND {$wpdb->posts}.post_type IN ( 'product', 'product_variation' ) AND ( {$wpdb->posts}.post_status = 'publish') )";
				}
			}
