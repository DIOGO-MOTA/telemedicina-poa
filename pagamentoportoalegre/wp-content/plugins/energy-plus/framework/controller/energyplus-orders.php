	...
	*/
	...
	public static function ajax() {
		...
		switch ($do){
			...
			if (in_array('wc-' . str_replace('wc-', '', $status), array_keys( wc_get_order_statuses() ))) {
				$filter['post_status'] = "wc-" . str_replace('wc-', '', $status);
			} else {
				$filter['post_status'] = array_keys( wc_get_order_statuses() );
			}
		...
		*/
		...
		private static function get_orders( $type, $filter = array(), $page = 0 ) {
			...
			if (!empty($filter['search']) && (2 < strlen($filter['search']) OR 0 ===  strlen($filter['search']))) {
				...
				if (apply_filters('energyplus_search_orders_by_sku', false)) {
					global $wpdb;

					$products_by_sku = $wpdb->get_var(
						$wpdb->prepare(
						...
						if ($products_by_sku) {
							$product = wc_get_product($products_by_sku);
							if ($product) {
								 $filter['search'] = html_entity_decode($product->get_name());
							}
						}
					}
