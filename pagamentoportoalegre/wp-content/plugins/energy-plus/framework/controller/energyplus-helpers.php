    ...
    if (!$need) {
      ...
      if ($query) {
        ...
        if ($first < (time()-(1*24*60*60))) {
          echo '<div class="__A__To_Activate">To activate your plugin, please <a href="'.EnergyPlus_Helpers::admin_page('reactors', array('action'=>'energy-activate')).'" class="trig text-warning">click here</a></div>';
        }
      }
    }
  }

  public static function ajax_enabled($role = 'manage_woocommerce') {
    return EnergyPlus::option('ajax_enabled', true) && current_user_can($role);
  }

  public static function hook_order_details($order_id) {
  }

  public static function style($values) {
    echo 'style' . '="'.esc_attr($values).'"';
  }
